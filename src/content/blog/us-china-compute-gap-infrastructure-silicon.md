...
- SMIC's **N+3 process (a DUV-based 7nm evolution reaching ~5nm-class logic density, not a native EUV 5nm node) reached volume production in December 2025**, confirmed by TechInsights teardown of the Kirin 9030 — no EUV involved ([TechPowerUp](https://www.techpowerup.com/344000/chinese-smic-achieves-5-nm-production-on-n-3-node-without-euv-tools), [TechInsights](https://library.techinsights.com/analysis-view/FCT-2512-802))
...
- Huawei's CloudMatrix 384 rack uses **~5.3x the accelerators (384 vs 72) for ~1.7x the dense system compute of an Nvidia GB200 NVL72, at roughly ~3.9x the rack power** — BF16 dense compute ~300 PFLOPs vs ~180 PFLOPs, ~559 kW vs ~145 kW ([SemiAnalysis](https://newsletter.semianalysis.com/p/huawei-ai-cloudmatrix-384-chinas-answer-to-nvidia-gb200-nvl72), [Tom's Hardware](https://www.tomshardware.com/tech-industry/artificial-intelligence/huaweis-new-ai-cloudmatrix-cluster-beats-nvidias-gb200-by-brute-force-uses-4x-the-power))
- China's LineShine supercomputer debuted at #1 on the June 2026 TOP500 with **2.198 exaflops (Linpack) using 13.79 million domestic CPU cores (LX2 304C processors) and no Western GPUs**, displacing US El Capitan ([TOP500](https://www.top500.org/system/180490/), [Forbes](https://www.forbes.com/sites/jonmarkman/2026/06/24/china-built-the-worlds-fastest-supercomputer-without-a-single-gpu/)). *Note: LineShine demonstrates domestic capability in traditional CPU-driven HPC and double-precision (FP64) workloads — frontier LLM training remains bound to dense matrix accelerators (GPUs/NPUs), so this is a distinct axis from AI cluster performance*
...
| Metric | US | China |
|---|---|---|
| Share of global GPU cluster performance (May 2025, Epoch AI) | ~75% | ~15% |
| Data center power demand | 31 GW (2025) → 66 GW (2027E) | Grid surplus, renewables being repurposed |
| National buildout plan | Private: Stargate $500B, xAI roadmap to 1M GPUs | State: ~$295B / 80% domestic tech |
| Publicly reported AI compute | No central registry (private labs) | 2,185-2,450 EFLOPS (FP16), +177% YoY |
| Anchor private commitments | xAI Colossus: 200k H100s live, 1M roadmap | Huawei/state hubs: 80% domestic chips target |

*Comparability note: the columns measure different things — Epoch's share data covers GPU clusters globally; China's official figures are FP16 "intelligent computing" capacity including domestic accelerators. They are consistent in direction, not directly convertible.*
...
**What's real:**
- **SMIC N+3 (DUV-based 7nm evolution, ~5nm-class density) is in volume production without EUV**, confirmed by TechInsights' Kirin 9030 teardown in December 2025 ([TechPowerUp](https://www.techpowerup.com/344000/chinese-smic-achieves-5-nm-production-on-n-3-node-without-euv-tools), [TechInsights](https://www.techinsights.com/blog/smic-n3-kirin-9030-pro-process-flow-analysis)), with a deeper process-flow analysis of the Kirin 9030 Pro (N+3) published August 2026 ([TechInsights](https://www.techinsights.com/blog/smic-n3-kirin-9030-pro-process-flow-analysis)). TechInsights' analysis is careful to call N+3 "a scaled evolution of its 7nm-class N+2 node" rather than a true 5nm node — in absolute terms it remains substantially behind leading-edge EUV nodes. Doing 5nm-class *density* with DUV multi-patterning is still an engineering achievement — it's just brutally expensive in wafer cost and yield, which is why SMIC is now raising prices into a shortage ([Tom's Hardware](https://www.tomshardware.com/tech-industry/semiconductors/smic-is-raising-wafer-prices-into-a-shortage-as-sanctions-wall-off-chinas-ai-demand))
...
- **At system level, Huawei compensates with optics and scale**: CloudMatrix 384 outperforms an Nvidia GB200 NVL72 — but using ~384 Ascend 910Cs vs 72 GB200s: roughly **5.3x the chips for ~1.7x the dense compute at ~3.9x the power** (BF16 ~300 vs ~180 PFLOPs; ~559 kW vs ~145 kW) ([SemiAnalysis](https://newsletter.semianalysis.com/p/huawei-ai-cloudmatrix-384-chinas-answer-to-nvidia-gb200-nvl72), [Tom's Hardware](https://www.tomshardware.com/tech-industry/artificial-intelligence/huaweis-new-ai-cloudmatrix-cluster-beats-nvidias-gb200-by-brute-force-uses-4x-the-power))
...
Domestic HBM (CXMT) is the long pole: yields are the variable "almost nobody is watching" but that decides the real compute ceiling ([insidecm](https://insidecm.substack.com/p/the-ascend-runs-on-a-warehouse-huaweis)). Until CXMT's HBM2/HBM3-class yields mature, every Ascend shipped depends on a depleting stockpile of foreign HBM. And the bottleneck isn't only the DRAM die itself — it's also **advanced 2.5D/3D packaging: the through-silicon via (TSV) and wafer-on-wafer stacking capability required to reliably stack memory layers** next to the logic die. Even if CXMT's DRAM yields catch up, packaging capacity and yields are a second, independent choke point on the same critical path ([SemiAnalysis](https://newsletter.semianalysis.com/p/huawei-ascend-production-ramp), [Tom's Hardware](https://www.tomshardware.com/tech-industry/artificial-intelligence/chinas-chip-champions-ramp-up-production-of-ai-accelerators-at-domestic-fabs-but-hbm-and-fab-production-capacity-are-towering-bottlenecks)).
...
| Capability | US | China |
|---|---|---|
| Leading-edge logic | TSMC 3nm-class + EUV | SMIC N+3 (DUV 7nm-evolution, ~5nm density, confirmed Dec 2025) |
| AI accelerators | GB300/Blackwell Ultra volume ramp | Ascend 910C volume; 920 reported |
| System efficiency | GB200 NVL72 baseline (~145 kW rack) | CloudMatrix 384: ~5.3x chips, ~1.7x dense BF16, ~3.9x power |
...
**Race 2 — efficiency per chip: the gap is not closing.** The CloudMatrix math (5.3x chips → 1.7x compute → 3.9x power) is the whole story in miniature. China can win on volume what it cannot win on unit economics. That matters enormously for two reasons:
